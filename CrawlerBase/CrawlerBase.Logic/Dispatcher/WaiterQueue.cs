...
{
    public class WaiterQueue<T> : BlockingQueue<T>
        where T : class
    {
        public WaiterQueue()
        {
        ...
        public T GetTimeout(int millisec)
        {
            try
            {
                return Dequeue(millisec);
            }
    ...
    /// </summary>
    public class BlockingQueue<T> : ICollection<T>
    {
        #region Fields
        private T[] buffer;			// Buffer used to store queue objects with max "Size".
        ...
        /// </summary>
        /// <param name="value"></param>
        private void Enqueue(T value, int millisecondsTimeout)
        {
            lock (syncRoot)
            {
                while (count == size)
        ...
        public void Clear()
        {
            lock (syncRoot)
            {
                ...
                // Pulse any producer(s) that may have been waiting on a full queue.
                Monitor.PulseAll(syncRoot);
            }
        }
